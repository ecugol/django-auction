...
class TestBaseClassMixin(object):
    ...
    def test_base_class(self):
        if self.model and self.base_class:
            self.assertEqual(self.model.__bases__[0].__name__, self.base_class)
...
class ModelTestMixin(TestBaseClassMixin):
    ...
    def test_meta(self):
        ...
        self.assertEqual(meta.app_label, self.app_label)
        self.assertEqual(meta.verbose_name, self.verbose_name)
        self.assertEqual(meta.verbose_name_plural, self.verbose_name_plural)
...
class BidBasketModelTests(TestCase, TestBaseClassMixin):
    ...
    def setUp(self):
        self.user = User.objects.create_superuser(
            "fubar", "fubar@example.com", "password"
        )
        self.user2 = User.objects.create_superuser(
            "randomperson", "randomperson@example.com", "somepassword"
        )

        now = auction.utils.generic.get_current_time()
        auction_name = "Cool auction"
        auction_name2 = "Awesome auction"
        self.auction, created = auction.models.Auction.objects.get_or_create(
            name=auction_name,
        ...
        self.auction2, created = auction.models.Auction.objects.get_or_create(
            name=auction_name2,
        ...
        self.lot, created = auction.models.Lot.objects.get_or_create(
            name=lot_name,
        ...
        self.lot2, created = auction.models.Lot.objects.get_or_create(
            name=lot_name2,
        ...
        self.lot3, created = auction.models.Lot.objects.get_or_create(
            name=lot_name3,
    ...
    def test_add_bid(self):
        ...
        self.assertEqual(biditem, returned_biditem)
    ...
    def test_update_bid(self):
        ...
        biditem = auction.models.BidItem.objects.filter(bid_basket=bidbasket)[0]
        self.assertEqual(biditem.amount, Decimal("42.00"))

        biditem = bidbasket.update_bid(biditem.pk, "69.69")
        self.assertEqual(biditem.amount, Decimal("69.69"))
    ...
    def test_update_bid_with_multiple_bid_items(self):
        ...
        biditem_1 = auction.models.BidItem.objects.get(
            bid_basket=bidbasket, lot_id=self.lot.pk
        )
        biditem_2 = auction.models.BidItem.objects.get(
            bid_basket=bidbasket, lot_id=self.lot2.pk
        )
        biditem_3 = auction.models.BidItem.objects.get(
            bid_basket=bidbasket, lot_id=self.lot3.pk
        )
        self.assertEqual(biditem_1.amount, Decimal("42.00"))
        self.assertEqual(biditem_2.amount, Decimal("1.00"))
        self.assertEqual(biditem_3.amount, Decimal("5.00"))
        ...
        self.assertEqual(biditem_1.amount, Decimal("40.00"))
        self.assertEqual(biditem_2.amount, Decimal("10.00"))
        self.assertEqual(biditem_3.amount, Decimal("50.00"))
    ...
    def test_delete_bid(self):
        ...
        self.assertEqual(biditem.lot.name, "Foo bar")
    ...
    def test_delete_bid_with_multiple_bid_items(self):
        ...
        self.assertEqual(len(biditems), 3)

        biditem = auction.models.BidItem.objects.get(
            bid_basket=bidbasket, lot_id=self.lot2.pk
        )
        ...
        biditems = auction.models.BidItem.objects.filter(bid_basket=bidbasket)
        self.assertEqual(len(biditems), 2)
    ...
    def test_add_bid_to_inactive_lot(self):
        ...
        a, created = auction.models.Auction.objects.get_or_create(
            name=auction_name,
            slug=slugify(auction_name),
            start_date=now,
            end_date=now + datetime.timedelta(365),
        ...
        lot, created = auction.models.Lot.objects.get_or_create(
            name=lot_name,
            slug=slugify(lot_name),
            active=False,
            content_type=ct,
